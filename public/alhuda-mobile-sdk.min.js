(function () {
  ...
  const containerIds = containerAttr.split(',').map(s => s.trim()).filter(Boolean);
  ...
  const post = async (url, data) => {
    if (!data.ad_id || !data.source_id) {
      console.warn('[Alhuda SDK] ⚠️ Missing ad_id or source_id in payload');
      return;
    }
    ...
    try {
      ...
      console.warn('[Alhuda SDK] ❌ Tracking failed:', err);
    }
  };
  ...
  const fetchAd = (format, el) => {
    ...
    fetch(`${sdkBase}/api/ad?key=${key}&format=${format}`, {
      ...
      .catch(err => {
        console.warn('[Alhuda SDK] ❌ Ad fetch failed:', err);
  ...
  formats.forEach(format => {
    containerIds.forEach(id => {
      ...
      if (el) fetchAd(format, el);
      else console.warn(`[Alhuda SDK] ⚠️ Container not found: ${id} or ${id}-${format}`);
    });
  });
})();
